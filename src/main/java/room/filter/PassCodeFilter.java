...
public class PassCodeFilter extends HttpFilter{
	...
	protected void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		HttpSession session = request.getSession(); // 取得session 物件
		System.out.println("PassCodeFilter for Room = " + session.getId());
		...
		}else if ("1234".equals(session.getAttribute("code")+ "")) { // 檢查 session ["code"] 的內容是否是1234
			chain.doFilter(request, response); // pass
		}else {
